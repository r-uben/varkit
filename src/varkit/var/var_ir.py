...
class ImpulseResponse:
    ...
    def __init__(self, var_results: VARResults, var_options: Dict):
    ...
    def PSI(self):
        if self.__PSI is None:
            self.__PSI, self.__Fp = self.get_wold_representation(self.var_options['nsteps'], self.var_options['recurs'])
        return self.__PSI
    ...
    def B(self):
        if self.__B is None:
            if self.var_options['ident'] == 'short':
                self.__B = VARUtils.get_cholesky_identification_short(self.var_results.sigma)
            elif self.var_options['ident'] == 'long':
                # B matrix is recovered with Cholesky on cumulative IR to infinity
                self.__B = VARUtils.get_cholesky_identification_long(self.var_results.sigma, self.var_results.Fcomp)
        elif self.var_options['ident'] == 'sign':
            # B matrix is recovered with sign restrictions
            if self.var_results.B is None:
                raise ValueError('You need to provide the B matrix with sign restrictions')
            self.__B = self.var_results.B
        
        elif self.var_options['ident'] == 'iv':
    ...
    def get_wold_representation(self, nsteps: int, recurs: str = 'wold') -> Tuple[pd.DataFrame, Dict[int, pd.DataFrame]]:
        ...
        if recurs == 'wold':
            ...
            params = self.var_results.varfit.params
            ...
            PSI = pd.DataFrame(
                np.zeros((self.var_results.nvar, self.var_results.nvar, nsteps)),
                index=pd.Index(self.var_results.varfit.model.endog_names, name='shock_variable'),
                columns=pd.Index(self.var_results.varfit.model.endog_names, name='response_variable')
            )
            PSI.iloc[:, :, 0] = np.eye(self.var_results.nvar)
            Fp = None  # No need to compute Fp if not using Wold
            
        return PSI, Fp

    def var_ir(self) -> np.ndarray:
        """Compute impulse responses (IRs) for a VAR model.
        ...
        iv = self.var_results.IV
        if self.var_options['ident'] == 'iv' and iv is None:
            raise ValueError('You need to provide the data for the instrument in VAR (IV)')
        

        shock_var = self.var_options['shock_var']

        # Retrieve and initialize variables
        nsteps = self.var_options['nsteps']
        impact = self.var_options.get('impact', 0)  # Default to 0 (one stdev shock)
        recurs = self.var_options.get('recurs', 'wold')  # Default to 'wold'
        Fcomp = self.var_results.Fcomp
        nvar = self.var_results.nvar
        nlag = self.var_results.nlag
        sigma = self.var_results.sigma

        # Get statsmodels VARResults object
        sm_var_results: VARResults = self.var_results.varfit
            ...
            for var in var_names:
                ...
                if recurs == 'wold':
                    ...
                    for step in range(1, nsteps):
                        Psi = self.PSI.loc[self.PSI['step'] == step, var_names]
                        Psi.index = var_names
